...
```
758 passed in 116.61s
```

758 tests in 44 files, none of which reads the disc. They cover the Python
...
| Installed | Result |
|---|---|
| everything (MSVC + capstone) | `758 passed` |
| no capstone — **what CI installs** | `739 passed, 1 skipped` |
| no MSVC | `590 passed, 168 skipped` |
| neither — **the Ubuntu CI leg** | `571 passed, 169 skipped` |
...
² 168 of the 758 skip without a C compiler; they build one runtime file and run it.
...
| **Tests** | ubuntu | `pytest`, `ruff check`, `ruff format --check` — 571 passed, 169 skipped |
| **Tests** | windows | the same three — 739 passed, 1 skipped |
