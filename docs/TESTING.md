...
```
849 passed in 147.92s
```

849 tests in 51 files, none of which reads the disc. They cover the Python
...
| `test_gxr_fastpath.py` | 2 | the pixel path's specialised cases (H15c) against the general path: 4,000 random register sets through the real `tev_prepare`, near misses included, 64 random pixels each through both TEV paths, and 400,000 random blends through both blend cases -- colour and alpha test identical |
...
| Installed | Result |
|---|---|
| everything (MSVC + capstone) | `849 passed` |
| no capstone — **what CI installs** | `830 passed, 1 skipped` |
| no MSVC | `614 passed, 235 skipped` |
| neither — **the Ubuntu CI leg** | `595 passed, 236 skipped` |
...
² 235 of the 849 skip without a C compiler; they build one runtime file and run it.
...
| **Tests** | ubuntu | `pytest`, `ruff check`, `ruff format --check` — 595 passed, 236 skipped |
| **Tests** | windows | the same three — 830 passed, 1 skipped |
