...
```
760 passed in 108.51s
```

760 tests in 44 files, none of which reads the disc. They cover the Python
...
| Installed | Result |
|---|---|
| everything (MSVC + capstone) | `760 passed` |
| no capstone — **what CI installs** | `741 passed, 1 skipped` |
| no MSVC | `590 passed, 170 skipped` |
| neither — **the Ubuntu CI leg** | `571 passed, 171 skipped` |
...
² 170 of the 760 skip without a C compiler; they build one runtime file and run it.
...
| **Tests** | ubuntu | `pytest`, `ruff check`, `ruff format --check` — 571 passed, 171 skipped |
| **Tests** | windows | the same three — 741 passed, 1 skipped |
