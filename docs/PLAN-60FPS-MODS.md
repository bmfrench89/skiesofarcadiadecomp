...
**H12. Texture hashing and the cache** — *done 2026-09-25: a texture is hashed once an epoch (BP 0x66, every EFB copy, a replay's RAM load), and the cache holds 1,024 behind an index. Prepare 3.9 → 0.21 ms a frame, prepare + decode 6.6 → 2.9 on H1's Part L run; +3.2% throughput with the clock out; paced fps −1.6%, the saved time waiting at H14's drains. `SOA_TEXVERIFY` found no rewrite inside an epoch in 2.57 M lookups over four scenes. FINDINGS "H12". Since, a palette load sends the textures it overlaps to be hashed again instead of throwing their decodes out, all of which came back unchanged: decode -79% in the Dangral base, fps unchanged (FINDINGS "Palette loads").*
...
**M3. Native mod code: `mod.dll` and `SoaModApi`** — *done 2026-09-25: `mod.dll` on `SoaModApi` v1 (`runtime/soa_mod.h`), the safe point, map loads and scene changes, `pad_filter`, the projection filter and the texture provider; `examples/mods/map-log`. The live title is no frame-hash oracle, so the visual filters were checked on the 23 pinned captures under `--replay`. Dropping START alone does not keep a run on the title (A also leaves the demo); dropping every button does. FINDINGS "M3a", "M3b", "M3c", "M3c, textures".*
