...
**M3. Native mod code: `mod.dll` and `SoaModApi`** — *M3a done 2026-09-25: `mod.dll` loading, `SoaModApi` v1 (`runtime/soa_mod.h`) with memory, state, log and four callbacks, `examples/mods/map-log`; over the title and a forced battle its `on_safe_point` calls equal the safe points and its `on_map_loaded` calls the field load lines. M3b done the same day: `pad_filter`; a filter dropping every button keeps the title run on the title's demo, and a pass-through one passes `title --check` (dropping START alone does not keep it there: A also leaves the demo). The renderer filters (M3c) remain. FINDINGS "M3a", "M3b".*
