...
| `[~]` | **7.4** Playthrough hardening | XL | The long tail. Save/load parity, every scene. **Started:** a save round-trips through Continue, the part select reaches parts B-L, the ending plays, and all 186 warpable maps below 500 load without a runtime fault |
...
| `[x]` | **8.3** Function swap-in harness | M | **Done:** decompiled functions are compiled natively (`dc_*`) and bound in `config/hle.txt` through adapters in `runtime/decomp_swap.c`, so the port runs them instead of the translation; the translation survives as `recomp_fn_*` and `SOA_SELFTEST=1` runs each pair on the same guest memory with random inputs (twelve functions, 200 rounds). Byte-oriented routines only until decompiled code reads wider fields through byte-order-aware accessors |
| `[~]` | **8.4** Decomp grind | XL | **Started:** 100 symbols match across 21 units -- 83 functions and 17 data (MSL string, memory, wide-char and numeric routines, the AR and ARQ libraries, the PowerPC architecture and matrix libraries, and six of the game's ARAM cache helpers) -- 0.29% of `.text`; twelve of them run in the port through `config/hle.txt`. 65 are compared in full and 35 carry words only a link can decide, which `tools/decomp.py` reports separately. The figure was 41 until the oracle stopped comparing relocated words on six bits. The game code is mwcc 1.3.2 `-O4,p` like its libraries; `src/sdk/ar/` needs 1.2.5n |
