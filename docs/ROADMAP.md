...
| `[~]` | **8.3** Function swap-in harness | M | **First form:** `src/` is also compiled natively (every function renamed `dc_*`) and linked into the port; `SOA_SELFTEST=1` runs each decompiled function against its recompiled twin on the same guest memory with random inputs (seven string routines, 200 rounds). Swapping a decompiled function in for its twin at run time is the remaining step |
