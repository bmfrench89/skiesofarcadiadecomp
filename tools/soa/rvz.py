...
class RVZ:
    def __init__(self, path):
        ...
        nrde,= struct.unpack('>I',h2[0xB4:0xB8]); rdeo,=struct.unpack('>Q',h2[0xB8:0xC0]); rdes,=struct.unpack('>I',h2[0xC0:0xC4])
        nge, = struct.unpack('>I',h2[0xC4:0xC8]); geo, =struct.unpack('>Q',h2[0xC8:0xD0]); ges, =struct.unpack('>I',h2[0xD0:0xD4])
    ...
    def _blob(self,off,size):
        self.f.seek(off); return zstd.decompress(self.f.read(size))
    def _table(self,off,size,n,esz,fmt):
        b=self._blob(off,size); return [struct.unpack(fmt,b[i*esz:(i+1)*esz]) for i in range(n)]

    def group(self, idx):
        if idx in self.cache: return self.cache[idx]
        doff,dsize,packed = self.groups[idx]
        comp = bool(dsize & 0x80000000); dsize &= 0x7FFFFFFF
        if dsize==0:
            out = b'\0'*self.chunk
        else:
            self.f.seek(doff*4); raw = self.f.read(dsize)
            data = zstd.decompress(raw) if comp else raw
            out = self._unpack(data) if packed else data
        out = out.ljust(self.chunk, b'\0')
        if len(self.cache)<64: self.cache[idx]=out
        return out
    ...
    def _unpack(self, d):
        ...
        while p+4 <= len(d):
            ...
            junk = sz & 0x80000000; sz &= 0x7FFFFFFF
            if junk:
    ...
    def read(self, offset, length):
        ...
        out=bytearray(); end=offset+length
