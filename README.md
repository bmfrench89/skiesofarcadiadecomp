...
| `SOA_HOSTPROF=1` | sample every rasterizer worker's instruction pointer about once a millisecond and end the report with where their time went, by function and by source line (through `gen/soa.pdb`, which `--link` writes). For finding what to make faster in the pixel path; a profiled run is for reading, not for timing |
...
```powershell
python -m pytest                     # 844 tests; any that need a dump skip themselves
