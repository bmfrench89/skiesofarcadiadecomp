...
| host buttons and chords | the pad's LB, View (Back) and stick clicks never reach the game: they are the port's (PLAN-GAMEPLAY-MODS CH1). View+LB is fullscreen, View+RS turbo and View+LS the menu -- each logs `[chord] frame F: ... (not built yet)` until its slice lands -- and LB alone is left to mods (`host_buttons` in `runtime/soa_mod.h`). The keyboard's Tab counts as LB. A pad script names them `lb`, `view`, `ls`, `rs`; a recording notes each chord as a `# chord` comment. Port 1 is the first connected XInput pad, in any slot |
